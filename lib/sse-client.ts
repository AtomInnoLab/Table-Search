...
export class SSEClient {
   ...
   * 发起GET SSE请求
   */
  async connect(url: string, options: SSEOptions = {}, extraHeaders?: Record<string, string>) {
    return this._run(
      () =>
        fetch(url, {
          method: 'GET',
          headers: { Accept: 'text/event-stream', 'Cache-Control': 'no-cache', ...extraHeaders },
          credentials: 'include',
   ...
   * 发起POST SSE请求
   */
  async connectPost(url: string, body: any, options: SSEOptions = {}, extraHeaders?: Record<string, string>) {
    return this._run(
      () =>
        fetch(url, {
          method: 'POST',
