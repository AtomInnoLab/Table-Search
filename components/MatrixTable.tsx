    ...
    return (
      <div className="text-center py-16">
        <div className="inline-flex items-center gap-2 text-gray-400">
          ...
          <span className="text-sm">{t('loadingPapers')}</span>
        </div>
      </div>
    )
  }
    ...
    <div className="w-full">
      ...
      {isExtracting && (
        <div className="mb-3 sm:mb-4 px-3 sm:px-4 py-2.5 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200/60 rounded-lg flex items-center gap-2.5">
          ...
          <span className="text-xs sm:text-sm font-medium text-blue-700">{t('extractingInfo')}</span>
        </div>
      )}
      ...
      {showNotify && notifyCount > 0 && (
        <div className="mb-3 flex justify-end">
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-yellow-50 border border-yellow-200 text-yellow-700 text-xs font-medium shadow-sm animate-row-highlight">
            ...
            {t('newPapersScreened', { count: notifyCount })}
          </span>
        </div>
      )}
      ...
      <div className="hidden md:block">
        <DesktopTable papers={papers} columns={columns} getCellData={getCellData} newPaperIds={newPaperIds} clearNewPaperId={clearNewPaperId} t={t} />
      </div>

      {/* Mobile: Cards */}
      <div className="block md:hidden">
        <MobileCards papers={papers} columns={columns} getCellData={getCellData} newPaperIds={newPaperIds} clearNewPaperId={clearNewPaperId} t={t} />
      </div>
      ...
      <div className="mt-2 flex items-center gap-3 text-xs text-gray-400">
        {totalSearched > 0 && (
          <>
            <span className="flex items-center gap-1">
              ...
              {t('statsSearched', { count: totalSearched })}
            </span>
        ...
        <span className="flex items-center gap-1">
          ...
          {t('dimensionCount', { count: columns.length })}
        </span>
      </div>
    </div>
  )
...
function DesktopTable({
  papers, columns, getCellData, newPaperIds, clearNewPaperId, t,
}: {
  papers: Paper[]
  columns: Column[]
  ...
  const tableColumns = useMemo<ColumnDef<TableRow, any>[]>(() => {
    const fixed: ColumnDef<TableRow, any>[] = [
      columnHelper.accessor('title', {
        id: '_title',
        header: () => <span className="whitespace-nowrap">{t('colTitle')}</span>,
        cell: (info) => (
    ...
    const dynamic: ColumnDef<TableRow, any>[] = columns.map((col) =>
      columnHelper.display({
        ...
        cell: (info) => (
          <CellRenderer cellData={getCellData(info.row.original.id, col.id)} t={t} />
        ),
...
const MobileCard = memo(function MobileCard({
  paper, columns, getCellData, index, isNew, clearNewPaperId, t,
}: {
  paper: Paper
  columns: Column[]
  getCellData: (paperId: string, columnId: string) => CellData | undefined
              ...
              <div key={col.id}>
                <div className="flex items-center gap-1.5 mb-0.5">
                  ...
                  {col.column_type === 'auto' && (
                    <span className="text-[8px] leading-3 font-bold bg-indigo-100 text-indigo-500 px-1 py-0.5 rounded">{t('badgeAuto')}</span>
                  )}
                  {col.column_type === 'custom' && (
                    <span className="text-[8px] leading-3 font-bold bg-emerald-100 text-emerald-600 px-1 py-0.5 rounded">{t('badgeCustom')}</span>
                  )}
                </div>
                <div className="text-[13px] leading-relaxed">
                  <CellRenderer cellData={cellData} t={t} />
                </div>
              </div>
            )
          })}
...
const CellRenderer = memo(function CellRenderer({ cellData, t }: { cellData?: CellData; t: TranslateFn }) {
  ...
  switch (cellData.status) {
    case 'loading':
      return (
        <div className="flex items-center gap-2 text-gray-300">
          ...
          <span className="text-xs">{t('cellExtracting')}</span>
        </div>
      )
    case 'completed':
      return (
    ...
    case 'na':
      return <span className="text-gray-300 italic text-xs">{t('cellNA')}</span>
    case 'error':
      return (
